...
func (m *MTask) SelectByID(id int64) (b *TaskDBType, err error) {
	...
	// считывание строки выборки
	err = row.Scan(&b.Pk_id, &b.Fk_status, &b.Fk_employee, &b.C_name, &b.C_sch_hours, &b.C_fact_hours)
	if err != nil {
		if err == sql.ErrNoRows {
			err = nil
			return
...
func (m *MTask) Insert(Task *TaskDBType) (id int64, err error) {
	...
	// выполнение запроса
	row = m.db.QueryRow(query, Task.Fk_status, Task.Fk_progect, Task.Fk_employee, Task.C_name, Task.C_sch_hours, Task.C_fact_hours)
...
func (m *MTask) Update(Task *TaskDBType) (err error) {
	...
	// выполнение запроса
	_, err = m.db.Exec(query, Task.Pk_id, Task.Fk_status, Task.Fk_progect, Task.Fk_employee, Task.C_name, Task.C_sch_hours, Task.C_fact_hours)
	if err != nil {
		if err == sql.ErrNoRows {
			err = nil
			return
...
func (m *MTask) Delete(Task *TaskDBType) (err error) {
	...
	// запрос
	query = `
	DELETE FROM task_manager.t_tasks 
	WHERE pk_id = $1;
	`
